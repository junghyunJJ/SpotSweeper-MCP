# Test the fixed gene symbol detection
            ...
            if (col %in% available_cols) {
                ...
                if (!is.null(gene_symbols) && !all(is.na(gene_symbols))) {
                    ...
                    if (length(unique(gene_symbols)) == length(gene_symbols)) {
                        ...
                        cat(sprintf("Column '%s' contains duplicates, will check for gene_id\n", col))
                    }
                    # Break the loop once we found a gene symbol column for mito detection
                    break
                }
            }
        }
...
# Verify that rownames are now gene_ids
cat("\n=== Verifying rownames ===\n")
cat("Rownames match gene_id column: ", all(rownames(spe_updated) == rowData(spe_updated)$gene_id), "\n")

# Test mitochondrial detection
